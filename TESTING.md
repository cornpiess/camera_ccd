...
**取景器（Color/Tone WYSIWYG：色彩/色调与成片一致；颗粒/halation/detail 仅成片）**
...
**旋转（canonical 相机交互）**
- 竖→横→竖来回旋转：整个界面跟随旋转（顶部胶囊/快门转到对应边），取景器横屏时变为全高 4:3 横幅；无抽搐、无 90° 跳变残留。
- 横持拍摄 → 相册照片为横向；竖拍为竖向。

**焦距切换（系统相机形态）**
- 焦距圆圈（13/26/35…）显示在取景框内底部、约 36pt 小圆；点选切换生效；不遮挡取景主体。

**相机皮肤（per-camera skin）**
- 切到 Leica M9：界面 chrome 呈质感深红（非大红），顶部徽章边线/焦段选中圈/光圈指针同色。
- 切到 Ricoh Negative：界面呈浅绿。切换相机时皮肤即时跟随。

**光圈滚轮（DEMO）**
- 14 Plus 等固定光圈机型默认可拧光圈环（无极连续、无档位锁死，1/3 档处有触感反馈），环上标 DEMO；拧动时光圈 SVG 叶片开合 + 取景辉光/星芒联动，成片仍为镜头真实光圈。
- 光圈刻度中心指针为 1.5pt 细线、严格居中、颜色=当前相机主题色。

**拍照保存**
- 拍照后照片进相册；若相册添加权限被拒，屏幕出现常驻胶囊「Photo saving is off — tap to open Settings」，点击可跳设置，授权后拍照成功即消失。
